...
전역 템플릿(`~/.claude/`)이 활성화돼 있어도, **본 프로젝트의 작업은 onyu 자체 agent(`.claude/agents/*`) + `prompt/feat-implement.md` 자율 파이프라인을 우선 사용한다.** 전역 템플릿의 generic agent(`planner` / `developer` / `backend-developer` / `frontend-developer` 등)는 onyu 작업에 직접 디스패치하지 않는다 — onyu 파이프라인이 PRD→userflow/usecase→DB설계→계획→구현→품질→테스트 + HITL 게이트 + 매니페스트 갱신까지 더 풍부하게 커버하기 때문.

- **기능 추가/수정**: `prompt/feat-implement.md` 자율 파이프라인 (Phase 0~7) — onyu 자체 agent 오케스트레이션
...
| developer (supervisor / 분배) | `prompt/feat-implement.md` 오케스트레이션 + `common-task-planner` |
