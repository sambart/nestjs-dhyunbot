...
1. 클라이언트 유효성 검사: 대기 채널 미선택, 안내 메시지 채널 미선택, 버튼 라벨 누락, 대상 카테고리 누락 시 오류 표시
2. 설정 내용을 API(`POST /api/guilds/{guildId}/auto-channel`)로 전송
3. 백엔드에서 AutoChannelConfig, AutoChannelButton, AutoChannelSubOption DB 저장
4. 안내 채널에 Embed + 버튼 메시지 전송 또는 갱신 (F-VOICE-009)
5. 저장 성공 시 "저장되었습니다." 인라인 메시지 표시 (3초 후 자동 소멸)
6. 기존 설정이 있으면 페이지 제목에 `(수정)` 표기
