...
- **접근 방식**:
  1. 음성 대시보드(`/dashboard/guild/{guildId}/voice`)의 UserRankingTable에서 유저 행 클릭 시 이동
  2. 대시보드 상단 또는 유저 상세 페이지 내 검색창에서 닉네임 또는 디스코드 ID 직접 검색 후 선택
...
| UI 요소 | 설명 |
|---------|------|
| 파이/도넛 차트 | 채널별 `channelDurationSec` 합계 비율 시각화. 범례에 채널명 표시 |
...
| UI 요소 | 설명 |
|---------|------|
| 채널명 | VoiceChannelHistory의 channel.channelName |
| 입장 시각 | `joinAt` (로컬 시간 포맷) |
| 퇴장 시각 | `leftAt` (로컬 시간 포맷, 아직 퇴장 전이면 "접속 중" 표시) |
| 체류 시간 | `duration` (초 기반 시:분:초 포맷) |
