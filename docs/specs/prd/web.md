...
## 관련 모듈
- `apps/web/app/page.tsx` — 랜딩 페이지 (F-WEB-001, F-WEB-018)
- `apps/web/app/components/LandingNav.tsx` — 랜딩 고정 네비게이션 컴포넌트 (F-WEB-018)
- `apps/web/public/landing/` — 랜딩 페이지 이미지 자산 (고양이 일러스트 + 아이콘 PNG 8종)
- `libs/i18n/locales/ko/web/landing.json` — 랜딩 i18n 한국어
- `libs/i18n/locales/en/web/landing.json` — 랜딩 i18n 영어
...
### 완료
- 랜딩 페이지 리디자인 (고정 네비, Hero, zig-zag 기능 섹션, 설정 가이드, CTA 밴드, Footer) — **F-WEB-001 갱신, F-WEB-018 신규**
- 글로벌 레이아웃 + 헤더
...
### F-WEB-001: 랜딩 페이지 (리디자인)

> 변경이력: [prd-changelog.md](../../archive/prd-changelog.md)

- **경로**: `/`
- **참조 디자인**: MEE6(https://mee6.xyz/ko) 스타일 — 라이트 테마 유지
- **이미지 자산**: `apps/web/public/landing/` 아래 PNG 8종 (고양이 일러스트 + 아이콘)
- **i18n**: `libs/i18n/locales/{ko,en}/web/landing.json`
- **FE 파일**: `apps/web/app/page.tsx`

#### 페이지 구조 (위 → 아래)

| 순서 | 섹션 | 설명 |
|------|------|------|
| 1 | 고정 네비게이션 | 스크롤 시 상단에 고정. Onyu 로고 + 링크 4개 (F-WEB-018 참조) |
| 2 | Hero | 좌: 제목·설명·배지·CTA 2개, 우: `01_scene.png` 일러스트 |
| 3 | 기능 섹션 (zig-zag) | 좌우 교차 레이아웃 6개 기능 블록 |
| 4 | 설정 가이드 (3-step) | 카드형 3단계 가이드 |
| 5 | CTA 밴드 | `07_cat_plain.png` 일러스트 + "지금 서버에 추가" 버튼 |
| 6 | Footer | 법적 링크 + 기능·대시보드 링크 컬럼 |

#### 섹션 2: Hero

| UI 요소 | 설명 |
|---------|------|
| 배경 | 연한 라이트 그라디언트 (예: `from-indigo-50 via-white to-sky-50`) |
| 좌측: 제목 | 서비스 메인 슬로건 텍스트 (대형) |
| 좌측: 설명 | `hero.description` i18n 키 |
| 좌측: 배지 | "완전 무료" 레이블 (`hero.badge`). 아이콘 또는 색상 강조 뱃지 |
| 좌측: CTA — 초대 | `hero.cta.invite` → 봇 초대 링크 (외부). 주 색상 채워진 버튼 |
| 좌측: CTA — 기능 보기 | `hero.cta.features` → `#features` 앵커 스크롤. 외곽선 버튼 |
| 우측: 일러스트 | `01_scene.png` (alt: "산/나무 배경 위 고양이들"). 모바일에서 Hero 하단에 배치 |
| 네비 고정으로 인한 상단 여백 | `pt-{nav-height}` 적용 필요 (F-WEB-018 참조) |

#### 섹션 3: 기능 섹션 (zig-zag)

id 앵커: `features`

6개 기능 블록을 홀수 번째는 "이미지 좌, 텍스트 우", 짝수 번째는 "텍스트 좌, 이미지 우"로 배치한다. 모바일에서는 세로 스택으로 전환한다.

| 순번 | 기능명 | 아이콘 | 대형 일러스트 | i18n 키 접두사 |
|------|--------|--------|--------------|---------------|
| 1 | 음성 통계 | `icon_trend.png` | — | `features.voiceStats` |
| 2 | 자동 채널 | `icon_mic.png` | — | `features.autoChannel` |
| 3 | AI 분석 | `icon_lightning.png` | — | `features.gemini` |
| 4 | 신규 멤버 | `icon_adduser.png` | `03_cat_newmember.png` | `features.newbie` |
| 5 | 웹 대시보드 | `icon_settings.png` | — | `features.dashboard` |
| 6 | 비활동 회원 관리 | `icon_settings.png` | — | `features.inactiveMember` |

각 기능 블록에 표시되는 텍스트:

| 텍스트 요소 | i18n 키 패턴 |
|------------|-------------|
| 기능명 (카드 제목) | `features.{key}.title` |
| 짧은 설명 | `features.{key}.description` |
| 긴 상세 설명 | `features.{key}.detail` |
| 아이콘 alt | 기능명과 동일 |
...
3개 카드형 블록으로 구성한다 (기존 텍스트 유지, 카드 시각 스타일 개선).

| 항목 | i18n 키 |
|------|---------|
| 섹션 제목 | `setup.sectionTitle` |
| 섹션 설명 | `setup.sectionDescription` |
| 1단계 — 봇 서버 추가 | `setup.step1.*` |
| 2단계 — 봇 역할 최상위 이동 | `setup.step2.*` |
| 3단계 — 웹 대시보드 설정 | `setup.step3.*` |

#### 섹션 5: CTA 밴드

| UI 요소 | 설명 |
|---------|------|
| 배경 | 채도 있는 배경 색상 또는 그라디언트. 라이트 테마 내 강조 밴드 |
| 일러스트 | `07_cat_plain.png` (alt: "고양이 장식 일러스트"). 우측 또는 배경 배치 |
| 제목 | `ctaBand.title` i18n |
| 설명 | `ctaBand.description` i18n |
| CTA 버튼 | `ctaBand.button` i18n → 봇 초대 링크 (외부). 대형 흰 배경 또는 강조색 버튼 |

#### 섹션 6: Footer

| UI 요소 | 설명 |
|---------|------|
| 로고 / 서비스명 | Onyu 텍스트 로고 또는 아이콘 |
| 기능 링크 컬럼 | `footer.features` 헤더 + 각 기능 앵커(`#features`) 링크 |
| 대시보드 링크 컬럼 | `footer.dashboardLink` 헤더 + 로그인 페이지(`/auth/discord`) 링크 |
| 법적 링크 | `footer.privacy` → `/privacy`, `footer.terms` → `/terms` |
| 저작권 | "© {year} Onyu" |

#### i18n 추가 키 목록

기존 키는 유지하며, 아래 키를 신규 추가한다.

**한국어 (`ko/web/landing.json`)**

| 키 | 값 |
|----|----|
| `hero.badge` | 완전 무료 |
| `features.voiceStats.detail` | 멤버별 일별·채널별 음성 활동 시간을 자동으로 수집합니다. 대시보드에서 트렌드 차트와 유저 랭킹을 한눈에 확인하세요. |
| `features.autoChannel.detail` | 지정된 대기 채널에 입장하면 즉시 또는 선택 화면을 통해 개인 채널이 생성됩니다. 모두 퇴장하면 자동으로 삭제됩니다. |
| `features.gemini.detail` | Google Gemini AI가 서버의 음성 활동 데이터를 분석하여 건강도 점수와 개선 제안을 생성합니다. 주간 자동 리포트도 지원합니다. |
| `features.newbie.detail` | 신규 멤버에게 미션을 부여하고 달성 여부를 자동으로 관리합니다. 캔버스 이미지 기반 진행 현황 표시도 지원합니다. |
| `features.dashboard.detail` | Discord 계정으로 로그인하면 모든 설정과 통계를 웹 브라우저에서 관리할 수 있습니다. 서버 소유자 전용 관리 대시보드를 제공합니다. |
| `features.inactiveMember.title` | 비활동 회원 관리 |
| `features.inactiveMember.description` | 장기간 음성 활동이 없는 멤버를 자동으로 분류합니다 |
| `features.inactiveMember.detail` | 설정한 기준 일수 동안 음성 활동이 없는 멤버를 완전 비활동·저활동·활동 감소 등 등급으로 자동 분류하고, 관리자 채널에 알림을 전송합니다. |
| `ctaBand.title` | 지금 바로 시작해보세요 |
| `ctaBand.description` | Onyu는 완전 무료입니다. 설치 후 5분 안에 음성 통계를 확인할 수 있습니다. |
...
**영어 (`en/web/landing.json`)** — 동일 키, 영문 값으로 추가.

#### 접근성 / 반응형

| 항목 | 요구사항 |
|------|---------|
| 이미지 alt | 모든 `<img>` / `<Image>`에 의미있는 alt 텍스트 필수 |
| 모바일 기능 섹션 | `md:` 이하에서 zig-zag 레이아웃 해제, 세로 스택으로 표시 |
| 모바일 네비게이션 | `md:` 이하에서 링크 메뉴 숨기고 햄버거 버튼 또는 간소화 메뉴 표시 |
| Hero top padding | 고정 네비 높이만큼 `pt` 적용 (`pt-16` 또는 네비 실제 높이에 맞춤) |

#### 제외 / 숨김

| 항목 | 처리 방법 |
|------|----------|
| Stats 숫자 섹션 | 미구현, 해당 섹션 없음 |
| FAQ 섹션 | 미포함 (기존 `/dashboard/guild/{guildId}/help` 페이지가 FAQ 역할 수행) |

### F-WEB-018: 랜딩 고정 네비게이션 (신규)

> 변경이력: [prd-changelog.md](../../archive/prd-changelog.md)

- **위치**: `apps/web/app/components/LandingNav.tsx` — 랜딩 페이지(`page.tsx`)에서만 렌더링
- **동작**: 스크롤 위치와 무관하게 뷰포트 상단에 고정 (`position: fixed`, `z-index` 상위). 스크롤 시 배경 불투명도 전환 애니메이션 (투명 → 흰 배경 + 하단 테두리)
...
|------|---------|----------|------|
| Onyu 로고/텍스트 | — | `/` (홈) | 좌측 고정 |
| 기능 | `nav.features` | `#features` 앵커 | 페이지 내 스크롤 |
| 설정 가이드 | `nav.setup` | `#setup` 앵커 | 페이지 내 스크롤 |
| 대시보드 | `nav.dashboard` | `/auth/discord` (로그인 페이지) | 외부 라우트 이동 |
| 서버 추가 (CTA) | `nav.invite` | 봇 초대 링크 (외부) | 강조 스타일 버튼 |

#### 반응형

| 뷰포트 | 동작 |
|--------|------|
| `md` 이상 | 로고 + 링크 4개 + CTA 버튼 전체 표시 |
| `md` 미만 | 로고 + CTA 버튼만 표시. 나머지 링크는 햄버거 메뉴 또는 생략 |

#### 관련 FE 파일

- `apps/web/app/components/LandingNav.tsx` — 고정 네비게이션 컴포넌트
- `apps/web/app/page.tsx` — 랜딩 페이지에서 `LandingNav` 렌더링
